...
(** Update physics with substeps based on time scale.
    Returns (new_world, all_collisions) *)
...
let find_collisions world =
  Engine.find_collisions world

(** Check if any collisions are occurring *)
let is_colliding world =
  List.length (find_collisions world) > 0
