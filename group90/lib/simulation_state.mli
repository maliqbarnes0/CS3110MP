...
(** [create_initial ()] is the initial simulation state with default
    parameters. *)
...
(** [set_collision_anims state anims] is [state] with collision animations
    set to [anims]. *)
val set_collision_anims : t -> collision_animation list -> t

(** [set_pending_params state params] is [state] with pending parameters set
    to [params]. *)
val set_pending_params : t -> planet_params list -> t

(** [apply_params state] is [state] with applied parameters set to the
    current pending parameters, marking them as the new baseline. *)
...
(** [load_scenario state scenario_name] is [state] prepared for loading
    scenario [scenario_name], with trails and animations cleared and the
...
(** [add_collision_animations state collision_pairs old_body_colors current_time]
    is [state] with new collision animations added for each pair in
    [collision_pairs], using [old_body_colors] to determine explosion colors
...
(** [update_trails_with_fading state new_positions current_time] is [state]
    with trails updated using [new_positions] for active bodies and orphaned
    trail fading managed based on [current_time]. *)
val update_trails_with_fading : t -> Vec3.v list -> float -> t

(** [get_trail_render_info trail current_time] is [(positions, alpha)] for
    rendering [trail], where [alpha] is 1.0 for active trails and fades out
    for orphaned trails based on [current_time]. *)
val get_trail_render_info : trail_state -> float -> Vec3.v list * float

(** [prune_empty_trails state] is [state] with trails that have fully faded
    out (empty orphaned trails) removed. *)
