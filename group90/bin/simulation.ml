...
let rec simulation_loop state camera theta phi radius =
  ...
  let state_after_physics =
    if state_after_keyboard.Simulation_state.paused then
      state_after_keyboard
    else
      update_physics_step state_after_keyboard
  in
...
and handle_slider_input state =
  ...
  let rec process_sliders i state_acc =
    if i >= 3 then state_acc  (* Process 3 planets *)
      ...
      let state_after_density = match Ui.check_slider_drag (sidebar_x + 50) (base_y + 60) 130 1e10 6e10 with
        | Some new_density -> Simulation_state.update_planet_density state_acc i new_density
        | None -> state_acc
      in

      (* Handle radius slider *)
      let state_after_radius = match Ui.check_slider_drag (sidebar_x + 50) (base_y + 110) 130 10. 40. with
        | Some new_radius -> Simulation_state.update_planet_radius state_after_density i new_radius
...
and handle_keyboard_input state =
  ...
  let state_after_scenario =
    if is_key_pressed Key.One then
      ...
      load_scenario_by_index state 1
    else if is_key_pressed Key.Three then
      load_scenario_by_index state 2
  ...
  let state_after_timescale =
    if is_key_pressed Key.Z then
      Simulation_state.set_time_scale state_after_reset (min (state_after_reset.time_scale *. 1.5) 20.0)
    else if is_key_pressed Key.X then
      Simulation_state.set_time_scale state_after_reset (max (state_after_reset.time_scale /. 1.5) 0.1)
    else
      state_after_reset
  in
...
and update_physics_step state =
  ...
  let state_with_trails = Simulation_state.update_trails_with_fading state_with_world new_positions current_time in
  ...
  let state_with_collisions =
    if List.length all_collisions > 0 then
      Simulation_state.add_collision_animations state_pruned all_collisions old_body_colors current_time
    else
      state_pruned
  in
...
and render_frame state camera =
  ...
  List.iteri (fun i trail ->
    let (positions, alpha) = Simulation_state.get_trail_render_info trail current_time in
    if List.length positions > 0 then begin
      let (r, g, b, a) = if i < List.length base_trail_colors then
        List.nth base_trail_colors i
      else
        (200, 200, 200, 100)
      in
  ...
  Ui.draw_ui is_colliding state.time_scale state.paused state.pending_params has_changes num_alive state.current_scenario;
